...
public class ArcProgressBar extends View {
   ...
   private String mArcText= "";
     ...
     * 线条宽度
     */
    private int mDottedLineWidth = 50;
    /**
     * 线条高度
     */
    private int mDottedLineHeight = 8;
    /**
     * 圆弧跟虚线之间的距离
     */
     ...
     * 进度文字大小
     */
    private int mProgressTextSize = 55;
    /**
     * 进度描述
     */
...
//                (float) (mArcRadius + bDistance) - 2 * (mTextPaint.descent() + mTextPaint.ascent()), mTextPaint);
        canvas.drawText(mArcText, mArcRadius - mTextPaint.measureText(mArcText) / 2,
                (float) mArcRadius  +mArcRadius/2 , mTextPaint);

        Log.i("arctext", (float) (mArcRadius + bDistance) - 2 * (mTextPaint.descent() + mTextPaint.ascent()) + "bDistance=="+bDistance+"===mArcCenterX==="+mArcCenterX+"===mArcRadius==="+mArcRadius);
    ...
    private void drawRunText(Canvas canvas) {
        String progressStr = this.mRealProgress + ".00";
    ...
    private void drawRunDottedLineArc(Canvas canvas){
        ...
        for (int i = 0; i < mProgress; i++) {
            mDottedLinePaint.setColor(setColor(i,mDottedLineCount));
